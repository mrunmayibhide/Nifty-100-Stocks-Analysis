...
#Cannot create table (need to give location)spark.sql("create external table if not exists Day_Data (ID string,dated string,close double,high double,low double,open double,volume int) ROW FORMAT DELIMITED FIELDS TERMINATED BY ',';")

#loading the pig data into database
spark.sql("LOAD DATA INPATH 'hdfs://cluster-group17-m/Pigresult1/part-m-00000' INTO TABLE Day_Data;")
...
total_data = spark.sql("select count(*) from day_data;").show()
stock_data_limit = spark.sql("select * from day_data limit 10;").show()